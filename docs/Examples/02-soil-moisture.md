...
def make_log_posterior(
    dr, synthetic_obs, n_pixels, prior_low: float, prior_high: float, likelihood_sigma: float
):
    π = np.pi
    σ = likelihood_sigma
    N = len(synthetic_obs)
...
log_posterior = make_log_posterior(
    dr, synthetic_obs, n_pixels, prior_low=_prior_low, prior_high=_prior_high, likelihood_sigma=5.0
)

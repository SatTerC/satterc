...
def aggregate_daily_to_weekly(daily_da: NDArray[np.float64]) -> NDArray[np.float64]:
    """Resamples daily xarray data to weekly mean."""
    return daily_da.mean()
...
def aggregate_weekly_to_monthly(weekly_da: NDArray[np.float64]) -> NDArray[np.float64]:
    """Aggregates weekly nodes into monthly nodes."""
    return weekly_da.mean()

...
- Model documentation pages follow `docs/models/splash.md`: overview (with the
  ...
  API. The three tables are generated by mkdocstrings from the signatures and
  docstrings, so do not restate in prose anything a table already says.
- A model's configurable settings live in a `<model>_params` node returning a
  `<Model>Params` TypedDict, which the model node unpacks. Descriptions and
  defaults go on that function, never on the TypedDict fields — conduit reads
  the defaults off its keyword-only arguments, and `get_model_params` in
  `scaffold/config_gen.py` reads both `<model>` and `<model>_params`.
- Model DAG graphs are generated, not committed: `just graph <model>` writes
  ...
  docs build always renders them fresh.

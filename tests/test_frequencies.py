...
from satterc.models import _time
...
class TestOneSourceOfTruth:
    ...
    def test_models_reexport_the_same_objects(self):
        """`models._time` must not redeclare them, or contracts could drift."""
        assert _time.DAILY is DAILY
        assert _time.WEEKLY is WEEKLY
        assert _time.MONTHLY is MONTHLY
